...
- the map camera begins at a macro scale tightly centered on a painterly, species-tinted seed with a tapered shell, seam, and highlight; fine, cool-toned mineral grains, a pebble, and nearby debris establish a distinct soil texture and a world only inches wide; it pulls back as the player grows, and its upper edge tracks just above the player's current height so neighboring trees reveal only roots and lower trunk at seed scale and progressively more structure as the player grows
