...
| **Seedling** | 1 season + 2 root zones + 2 leaf growths | Root extension, fungal connections, then optional height growth | Aphids, browsing animals |
| **Sapling** | 4 seasons + survive 1 major event | Branch growth immediately; taproot, canopy, ally aid, shading, bark, and rhizosphere emerge across the first five Sapling seasons | Wind, competition |
| **Small Tree** | 2 years + 2 branches | Flowers immediately; thorns, toxic foliage, and trunk thickening emerge across the first three Small Tree seasons | Lightning, disease, first human surveys |
...
- capabilities are intentionally staggered instead of arriving in large stage-transition bundles: Sapling discoveries continue for five seasons, Small Tree discoveries for three seasons, and Mature Tree discoveries for three seasons; each delayed action uses the normal unlock announcement when it becomes available
...
- Grow Taller lengthens the trunk, provides one additional point of sunlight gathering, and raises the player's competitive canopy height, but each unbraced spindly level adds storm damage; Thicken Trunk braces one spindly level
