...
function failEncounter(state, encounter) {
  if (encounter.phase === 'survey') {
    ...
    return {
      ...
      body: '<p>The humans paint a bright mark low on your trunk. They leave for now, but the fungal network carries an uneasy certainty: they intend to return.</p>',
    };
  }
  ...
  if (state.cuttingProgress >= 3) {
    ...
    return {
      title: 'Felled',
      body: '<p>The final cuts pass through your remaining wood. Your crown falls, and your root-signals break into silence.</p>',
      fatal: true,
    };
  }
  return {
    title: 'A Deep Cut',
    body: `<p>The humans carve deeply into your trunk before leaving. You lose <strong>${damage} health</strong> and one level of trunk growth.</p><p><strong>Cutting damage:</strong> ${state.cuttingProgress}/3</p>`,
  };
}
...
export function resolveHumanDecision(state, decision, optionId, deps = {}) {
  ...
  if (decision.kind === 'conservation-inspection') {
    ...
    return {
      ...
      body: '<p>The visitors recognize that this is not merely valuable timber, but a rare community of great trees supporting one another across generations.</p><p>Boundaries are drawn around the grove. Cutting is forbidden. Your ecosystem will be allowed to endure.</p>',
      victory: true,
    };
  }
  ...
  if (option.id === 'drop-branch' && state.branches > 0) {
    ...
    return {
      title: 'Branchfall',
      body: '<p>You release a heavy branch. It crashes into the ground beside the humans, who abandon their tools and flee.</p><p>You lose <strong>1 branch</strong> and up to <strong>2 leaf clusters</strong>.</p>',
      repelled: true,
    };
  }
  ...
  if (random() < successChance) {
    ...
    return {
      ...
      body: option.id === 'call-network'
        ? '<p>Alarm pulses race through the connected grove. Roots shift, branches move, and the humans find themselves surrounded by signs of a living forest. They retreat.</p>'
        : '<p>Your thorns, toxic foliage, and imposing movement make the work too dangerous. The humans withdraw from the grove.</p>',
      repelled: true,
    };
  }
...
export function advanceHumanSystem(state, deps = {}) {
  ...
  return {
    event: {
      text: phase === 'survey'
        ? 'Humans have entered the grove and are measuring your trunk. They remain visible on the map.'
