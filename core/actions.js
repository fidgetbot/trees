...
export function createActions(deps) {
  ...
  return [
    ...
    { key: 'aidAlly', name: 'Offer Aid to Ally', icon: '🎁', category: 'diplomacy', help: 'Send substantial reserves to support an ally and improve its health.', baseCost: { sunlight: 0, water: 1, nutrients: 4 }, prereq: s => s.allies >= 1, effect: s => offerAidToAlly(s) },
    ...
    { key: 'shadeRival', name: 'Shade Neighbor', icon: '☂️', category: 'diplomacy', help: 'Crowd a neighboring tree out of light. Starting a fight gives less immediate payoff than pressing an existing rivalry.', baseCost: { sunlight: 3, water: 1, nutrients: 2 }, prereq: s => s.neighbors.some(n => !n.dead), effect: s => shadeRivalAction(s) },
